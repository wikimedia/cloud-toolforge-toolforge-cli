#!/bin/bash
set -eo pipefail
# Script to run CI checks on your local toolforge-cli code.
# It uses the same docker image we use to run such tests in
# CI.

usage() {
    cat <<USG
$0 - run tests on your toolforge-cli working directory.
...
IMG_NAME=${IMG_NAME:-docker-registry.wikimedia.org/releng/tox-poetry-buster:$IMG_VERSION}
CONT_NAME=toolforge-cli-tests-${IMG_VERSION}
